...
class SimpleRAG:
    ...
    def __init__(self, path, chunk_size=1000, chunk_overlap=200, n_retrieved=2):
        ...
        self.llm = get_langchain_model_provider(ModelProvider.GEMINI)

    def run(self, query):
        """
        Retrieves context and generates an answer for the given query using Gemini LLM.

        Args:
            query (str): The query to retrieve context for and generate an answer.
        """
        ...
        start_gen = time.time()
        chain = create_question_answer_from_context_chain(self.llm)
        result = answer_question_from_context(query, context, chain)
        self.time_records['Generation'] = time.time() - start_gen
        print(f"Generation Time: {self.time_records['Generation']:.2f} seconds")
        print(f"\n--- Generated Answer ---\n{result['answer']}\n")
